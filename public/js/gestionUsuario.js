...
function ingresarUsuario(){
    ...
    $.ajaxSetup({
        ...
        var FrmData = {
            ...
            departamento_id:$('#Usuario_departamento').val(),
            email:$('#email').val(),
...
function  cargartablaIusuario(data){
  //debugger
    $("#tablaUsuario").append(
        ...
        <td class='row'><button type='button' class='btn btn-warning' data-toggle='modal' data-target='#actualizarUsuariomodal' onClick='prepararactualizarusuario("+data.id+")'><i class='fa fa-edit'></i></button></td>\
        ...
        <td class='row'><button type='button' class='btn btn-info' id='btn-confirm' onClick='Asignar_Departamento_Cargo_al_Usuario("+data.id+")'><i class='fa fa-building-o'></i></button></td>\
        </tr>"
    );
...
function Asignar_Departamento_Cargo_al_Usuario(id){
    $('#ventanaAsignarCargosDepartamento').modal('show');
    $('#id_cargo_usuario').val(id);
    llenarCargoUsuario(); 
...
$('#cargousuario_fechaFin').change(function () {
   
    if ($('#cargousuario_fechaInicio').val()<=$('#cargousuario_fechaFin').val()) {
        
    }else{
        alert('Seleccione una fecha valida');
        $('#cargousuario_fechaFin').val($('#cargousuario_fechaInicio').val());
    }
 
 });

 function limpiarCargoUsuario(){
    $('#cargousuario_fechaInicio').val("");
    $('#cargousuario_fechaFin').val("");
        ...
        if (control =='0') {
           ...
           $.ajax({
               ...
               success: function (data)   // Una función a ser llamada si la solicitud tiene éxito
               {
                       ...
                       mostrarCargoUsuarios( $('#id_cargo_usuario').val());
...
function eliminarCargoUsuario(id){
    ...
    $.ajax({
        ...
        success: function (data)   // Una función a ser llamada si la solicitud tiene éxito
        {   
          mostrarCargoUsuarios( $('#id_cargo_usuario').val()); // carga los datos en la tabla                       
        }
    });
}
